...
std::vector<float> Response::get_eff_area()
{
	// Make vector to store effective area, it is energy bins equal to the number of instrument channels
	std::vector<float> eff_area (num_chans);
	// Each column will be summed to find the effective area, this is the temporary sum variable
	double tmp_col_sum = 0;

	for(int i = 0; i<num_chans; ++i)
	{
		// Reset summation
		tmp_col_sum = 0;
		// Sum the probability matrix along the photon energy axis
		for(int j = 0; j<num_phot_bins; ++j )
		{
			tmp_col_sum += prob_matrix.at(j).at(i);
		}
		eff_area.at(i) = tmp_col_sum;
	}
