...
class PromptJet(object):
	...
	def __init__(self,numshells=5000,dte=0.002,shelldist=sd.step,alpha_e=1/3,alpha_m=1,alpha_b=1/3,ksi=1e-3,mu=1.74,E_dot_iso=1e53,tb=0,theta=0.1,r_open=1e6,r_sat=1e8,eps_th=0.03,sigma=0.1):
		...
		if hasattr(dte,"__len__"):
			...
			self.dte_step = np.abs(dte[0]-dte[1])
	...
	def jet_evolution(self,jet_shells=None,alpha_e=None,alpha_m=None,alpha_b=None,ksi=None,mu=None,E_dot_iso=None,tb=None,theta=None,r_open=None,r_sat=None,eps_th=None,sigma=None):
		...
		T0 = np.power(E_dot_iso*theta**2 / (4*np.pi * cc.a * cc.c * r_open**2),1/4) # K, rearrangement Eq. 1 of Hascoet 2013 
		...
		spec_therm['te'] = t_cross_phot # time that the shell crosses the photosphere 
		spec_therm['ta'] = t_obs # Time that the arrives at observer 
		spec_therm['delt'] = del_t_obs # The width of the emission 
		spec_therm['T'] = T_phot_obs # Temperature of the shell at time of crossing
		spec_therm['Flux'] = L_phot # Luminosity or emission 
		spec_therm['Rphot'] = r_phot # radius of crossing the photosphere 
		...
		while ord_lorentz == False:
			...
			for i in range(len(active_shell_inds)-1):
				...
				if jet_shells['GAMMA'][ind_ds_shell_tmp] < jet_shells['GAMMA'][ind_us_shell_tmp]: 
					...
					tmp_ds_b = ut.beta(jet_shells['GAMMA'][ind_ds_shell_tmp])
			...
			for i in range(len(jet_shells)):
				...
				if jet_shells[i]['STATUS'] == 1:
					jet_shells[i]['RADIUS'] += ut.beta(jet_shells[i]['GAMMA'])*t_coll_lowest
