...
void make_thermal_spec(Spectrum * spectrum, float ta[], float Temp[], double Flux[], float delt[], int num_lines, float Tmin, float Tmax, bool nuFnu = true);
// Calculate the thermal spectrum from the given temperature and flux of the emission
void calc_thermal_spec(Spectrum * spectrum, float temp, double flux, bool nuFnu = true);
...
void make_synch_spec(Spectrum * spectrum, float ta[], float Esyn[], double e_diss[], float delt[], float tau[], float relvel[], int num_lines, float Tmin, float Tmax, bool nuFnu = true);
// Calculate the synchrotron spectrum from the synchrotron energy and flux of the emission
void calc_synch_spec(Spectrum * spectrum, float Esyn, double e_diss, float delt, bool nuFnu = true);
